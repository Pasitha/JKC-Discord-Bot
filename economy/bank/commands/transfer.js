...
module.exports.run = async (client, message, args) => {
    ...
    if (account_1.coins > 0) {
        ...
        if (!user || user.id === message.author.id) {
            ...
            let result = /([0-9]+)/.exec(args[1]);
            if (result[1] !== result.input) return message.channel.send({ embeds: [new MessageEmbed().setAuthor(`🤨เอ่ออ คุณ${message.author.username} คะ ที่ใส่มามันไม่ใช่ตัวเลขนะคะ รบกวนใส่ใหม่อีกทีนะคะ`).setColor('#ff0000')] });
