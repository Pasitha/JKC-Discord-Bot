...
module.exports.run = async (client, JKCJrBot, JKCSupBot, message, args) => {
    ...
    if (message.guild.id === '423007343127822338') {
        ...
        if (!args[0]) return message.channel.send({ embeds: [
            new MessageEmbed().setTitle('Jukucrush junior SS.5').setDescription(`${JKCSupBot.username}`).addField('ตอนนี้ในเซิฟอยู่ทีพิกัด', `x:${JKCSupBot.entity.position.x.toFixed(2)}, y:${JKCSupBot.entity.position.y.toFixed(2)}, z:${JKCSupBot.entity.position.z.toFixed(2)}`)
